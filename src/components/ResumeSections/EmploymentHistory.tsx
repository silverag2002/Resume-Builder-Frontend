...
export default function EmploymentHistory({ handleClick }: any) {
  ...
  return (
    <>
      <div
        ...
        onClick={(event) => handleClick(event, setShowEmploymentHistory)}
        id="employment-history"
      >
        <span id="employment-history">Employment History</span>{" "}
        <div className="border-x border-y rounded-md border-violet-600 p-2">
          <svg
            ...
            className={`w-6 h-6 text-violet-600  transition-all duration-500 ease-in-out ${
              showEmploymentHistory ? "rotate-180" : ""
            }`}
            id="employment-history"
          >
            <path
      ...
      <div
        className={`w-full h-1 ${
          showEmploymentHistory ? "hidden" : ""
        } bg-gray-200`}
      ></div>
      <div
        className={` rounded-md  transition-all duration-500 max-h-0 overflow-hidden ${
          showEmploymentHistory ? "max-h-full p-8 border-2" : ""
        }`}
      >
